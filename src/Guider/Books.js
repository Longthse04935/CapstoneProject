	...
	render() {
		let order = this.state.orders.map((order, index) =>
			<tr className="row100 body" key={index}>
				<td className="cell100 column2"><Link to={`/rumor/${order.traveler_id}`}>{order.object}</Link></td>
				<td className="cell100 column3">{order.begin_date}</td>
				<td className="cell100 column4">{order.finish_date}</td>
				<td className="cell100 column5"><Link to={`/posttour/${order.post_id}`}>{order.postTitle}</Link></td>
			...
			switch (this.state.status) {
				case "WAITING":
					return (<tr className="row100 body" key={index}>
						<td className="cell100 column1"><button className="accept" value={order.trip_id} id={index} onClick={this.accept} type="button">Accept</button>
							<button onClick={this.deny} value={order.trip_id} id={index} className="refuse" type="button">Refuse</button></td>
